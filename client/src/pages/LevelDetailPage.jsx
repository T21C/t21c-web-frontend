...
import { fetchLevelInfo, getColorDiff } from "../Repository/RemoteRepository";
    ...
    <div className="level-detail">
      ...
      <div className="wrapper-level wrapper-level-top">
        ...
        <div className="header">
          <div className="left">
            {res && (
              <div className="diff" style={{ backgroundColor: `#${getColorDiff(res.level.diff)}` }}>
                <p>{res && res.level.pguDiff}</p>
              </div>
            )}


            <div className="title">
              <h1>{res && res.level.artist} - {res && res.level.song}</h1>
              {res && (
                <p style={{ color: 'var(--gray-color)' }}>{id}</p>
              )}
            </div>
          </div>

          <div className="right">
            {res && res.level.dlLink !== "" && (
              <a href={res && res.level.dlLink}>
            ...
            {res && res.level.workshopLink !== "" && (
              <a href={res && res.level.workshopLink}>
            ...
            {(!res || (res.level.dlLink === "" && res.level.workshopLink === "")) && (
        ...
        <div className="body">
          ...
          <div className="youtube">
            {res && res.level.vidLink !== "" && (
              <iframe
                src={res && res.level.vidLink}
            ...
            {res && res.level.vidLink === "" && (
              // not responsive yet
              <img style={{ width: "40rem", height: "27rem" }} src={`src/assets/waves/${Math.floor(Math.random() * 4) + 1}.png`} alt="" />
            )}
        ...
        <div className="rank">
          ...
          <div className="rank-list">
            {res && res.passes && res.passes.count ? (
              ...
              <h1>This Level Havent Been Beaten Yet</h1>
            )}
          </div>
        </div>
      </div>
