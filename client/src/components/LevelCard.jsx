...
import { getColorDiff } from "../Repository/RemoteRepository";

const LevelCard = ({ object }) => {
  let navigate = useNavigate();

  const redirect = () => {
    navigate(`/leveldetail?id=${object.id}`);
  };
  ...
  return (
    <div className="grid-content-long">
      ...
      <div className="content" onClick={() => redirect()} style={{backgroundColor:`#${getColorDiff(object.diff)}`, borderRadius:"35px", border:"2px solid white"}}>
        {object.diff}
      </div>

      <div className="content content-download">
        {object.wsLink ? (
          <a href={object.wsLink} target="_blank">
            <svg
