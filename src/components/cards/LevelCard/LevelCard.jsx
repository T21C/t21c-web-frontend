...
const LevelCard = ({
  ...
  const renderClearedCheckmark = ({ className = '', noHover = false } = {}) => {
    ...
    const isPurePerfect = !!packItem.isPurePerfect;
    return (
      <svg
        className={`level-card__cleared${isPurePerfect ? ' pure-perfect' : ''}${noHover ? ' no-hover' : ''} ${className}`}
        width="16"
        height="16"
        viewBox="0 0 24 24"
        fill={isPurePerfect ? 'rgb(255, 233, 0)' : '#4CAF50'}
      >
