...
const PassDetailPage = () => {
  ...
  return (
    <>
          <MetaTags
        title={`${pass.level?.song} - ${pass.level?.artist} | TUF`}
        description={`View the pass of ${pass.player?.name} for ${pass.level?.song} - ${pass.level?.artist}`}
        url={currentUrl}
      ...
      {pass?.isDeleted && (
        <div className="deletion-banner-wrapper">
          <div className="deletion-banner">
            ...
            <span>This pass has been deleted</span>
          </div>
        </div>
      )}
        ...
        <div className="header">
          <div className="level-card">
            <div className="difficulty-section">
              ...
              {baseScore && <span className="base-score">{baseScore}PP</span>}
            </div>
            <div className="level-info">
              ...
              <div className="credits">
                {pass.level?.team && <p className="team">{pass.level?.team}</p>}
                <p className="charter">Chart by {pass.level?.charter}</p>
                {pass.level?.vfxer && <p className="vfxer">VFX by {pass.level?.vfxer}</p>}
              </div>
            </div>
          </div>
                ...
                {pass.scoreInfo && (
                  <div className="score-container">
                    <div className="current-score">
                      <span className="score-value">{pass.scoreInfo.currentRankedScore.toFixed(2)}</span>
                      {pass.scoreInfo.scoreDifference > 0 && (
                        <span className="score-difference positive">
                          +{pass.scoreInfo.scoreDifference.toFixed(2)}
                        </span>
                      )}
                    </div>
                    <div className="previous-score">
                      {pass.scoreInfo.previousRankedScore.toFixed(2)}
                    </div>
                  </div>
                )}
              </div>
              <div className="pass-date">Cleared on {formatDate(pass.vidUploadTime)}</div>
            </div>
          </div>
        </div>
          ...
          <div className="info">
            <div className="info-items">
              <div className="info-item">
                <p>Score</p>
                <span className="info-desc">{pass.scoreV2.toFixed(2)}</span>
              </div>
              <div className="info-item">
                <p>Accuracy</p>
                <span className={`info-desc ${accuracy === 1 ? 'perfect-shine' : ''}`}>{(100*accuracy).toFixed(2)}%</span>
              </div>
              <div className="info-item">
                <p>Speed</p>
                <span className="info-desc">{pass.speed}x</span>
              </div>
              <div className="info-item">
                <p>Feeling Rating</p>
                <span className="info-desc">{pass.feelingRating || 'None'}</span>
              </div>
            </div>
            {(pass.isWorldsFirst || pass.is12K || pass.is16K || pass.isNoHoldTap) && (
              <div className="flags-container">
                {pass.isWorldsFirst && (
                  <span className="worlds-first">🏆&nbsp; World's First! &nbsp;🏆</span>
                )}
                {(pass.is12K || pass.is16K || pass.isNoHoldTap) && (
                  <div className="flags">
                    {pass.is12K && <span className="flag">12K</span>}
                    {pass.is16K && <span className="flag">16K</span>}
                    {pass.isNoHoldTap && <span className="flag">No Hold Tap</span>}
                  </div>
                )}
              </div>
            )}
              ...
              <div className="judgement-grid">
                <div className="top">
                  <div className="judgement-item early-perfect">
                    <label>Early Perfect</label>
                    <span>{pass.judgements?.ePerfect || 0}</span>
                  </div>
                  <div className={`judgement-item perfect ${accuracy === 1 ? 'perfect-shine' : ''}`}>
                    ...
                    <div className="value-container">
                      ...
                      <span>{pass.judgements?.perfect || 0}</span>
                    </div>
                  </div>
                  <div className="judgement-item late-perfect">
                    <label>Late Perfect</label>
                    <span>{pass.judgements?.lPerfect || 0}</span>
                  </div>
                </div>
                <div className="bottom">
                  <div className={`judgement-item too-early ${pass.judgements?.earlyDouble === 0 && pass.level?.difficulty?.name[0] !== "P" && accuracy !== 1 ? 'zero-shine' : ''}`}>
                    <label>Too Early</label>
                    <span>{pass.judgements?.earlyDouble || 0}</span>
                  </div>
                  <div className="judgement-item early">
                    <label>Early</label>
                    <span>{pass.judgements?.earlySingle || 0}</span>
                  </div>
                  <div className="judgement-item late">
                    <label>Late</label>
                    <span>{pass.judgements?.lateSingle || 0}</span>
                  </div>
                  <div className="judgement-item too-late">
                    <label>Too Late</label>
                    <span>{pass.judgements?.lateDouble || 0}</span>
                  </div>
                </div>
              </div>
            </div>
